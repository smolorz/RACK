...
nohelp_text[] = N_(
	"There is no help available for this RACK option.\n"),
load_config_text[] = N_(
...
load_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep several different RACK\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than the\n"
	"RACK's default, entering the name of the file here will allow you\n"
...
save_config_help[] = N_(
	"\n"
	"For various reasons, one may wish to keep different RACK\n"
	"configurations available on a single machine.\n"
	"\n"
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, _("RACK %s Configuration"),
		sym_get_string_value(sym));
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(_("Do you wish to save your new RACK configuration?"));
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, _("\n\n"
				"Error during writing of the RACK configuration.\n"
				"Your RACK configuration changes were NOT saved."
				"\n\n"));
			return 1;
		}
		printf(_("\n\n"
			"*** End of RACK configuration.\n"
			"*** Execute 'make' to build RACK or try 'make help'."
			"\n\n"));
	} else {
		fprintf(stderr, _("\n\n"
			"Your RACK configuration changes were NOT saved."
