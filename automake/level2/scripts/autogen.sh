...
PROJECT_CONFIG=`find config -name "*config_pre.h.in"`
...
for FILE in aclocal.m4 configure $PROJECT_CONFIG; do
    if test -f $FILE; then
	rm -f $FILE
    fi
	echo -n .
...
autoheader
automake-1.9 --add-missing --copy --gnu -Wall -Wno-override -Wno-portability
autoconf -Wall
